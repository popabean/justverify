#!/usr/bin/env python3
"""Run signed policy behavior against every checksum-verified ARM Core release."""
import hashlib,json,pathlib,subprocess,sys
root=pathlib.Path(__file__).resolve().parents[1];out=root/'docs/evidence/policy-semantics';out.mkdir(parents=True,exist_ok=True)
private=root/'.state/policy-semantics';private.mkdir(parents=True,exist_ok=True);private.chmod(0o700)
source=hashlib.sha256((root/'tests/policy_semantics.py').read_bytes()).hexdigest();results=[]
for release in json.loads((root/'catalog/releases.json').read_bytes())['releases']:
 ...
 if release['verification']!='PASS':
  ...
  if previous.get('status')=='PASS' and previous.get('test_sha256')==source and previous.get('binary_sha256')==release['arm64_binary_sha256']:code=0
   ...
   with (private/(version+'.log')).open('wb') as log:
    run=subprocess.run([sys.executable,str(root/'tests/policy_semantics.py'),'--core-bin',str(binary.parent),'--report',str(report)],stdout=log,stderr=subprocess.STDOUT);code=run.returncode
 ...
 (root/'docs/evidence/policy-semantics-matrix.json').write_text(json.dumps(results,indent=2)+'\n')
