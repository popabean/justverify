...
fn real_resource_preflight_and_roundtrip() -> Result<()> {
    let root = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let binaries = PathBuf::from(std::env::var("JV_CORE_MATRIX")?);
    let state = root
        .join(".state")
        .join(format!("resource-preflight-{}", std::process::id()));
    ...
    for release in releases["releases"].as_array().unwrap() {
        ...
        let values = Values::from([
            ("bantime".into(), "120".into()),
            ("maxconnections".into(), "16".into()),
            ("maxreceivebuffer".into(), "4096".into()),
            ("maxsendbuffer".into(), "2048".into()),
            ("peertimeout".into(), "45".into()),
            ("timeout".into(), "1500".into()),
            ("maxuploadtarget".into(), "10".into()),
            ("dbcache".into(), "32".into()),
            ("rpcworkqueue".into(), "17".into()),
        ]);
        let plan = policy.preview(&file, values.clone())?;
        let receipt = policy.preflight(&binary, &state, &plan)?;
        assert_eq!(receipt.resources.len(), 11);
        policy.apply(&file, &plan, &receipt, || Ok(()))?;
        assert_eq!(policy.current(&file)?, values);
        println!(
            "{version}: nine resource configurations, effective connection limit/upload bytes, file roundtrip PASS"
        );
        tested += 1;
    }
    assert_eq!(tested, 32);
